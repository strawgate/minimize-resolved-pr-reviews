          ...
          pullRequestReview:
            opts.reviewId === null
              ...
              : {
                  ...
                  createdAt: opts.reviewCreatedAt ?? "2025-01-01T00:00:00Z",
...
describe("groupThreadsByReview", () => {
  ...
  it("extracts isMinimized from the review", () => {
    const threads = [makeThread({ reviewId: "r1", reviewIsMinimized: true })];
  ...
  it("preserves isResolved on thread info", () => {
    const threads = [makeThread({ isResolved: true, reviewId: "r1" })];
