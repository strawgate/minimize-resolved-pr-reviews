...
THREADS_JSON='.data.repository.pullRequest.reviewThreads.nodes[].comments.nodes[]'

# Count matching comments to guard against vacuous-truth passes
OLD_COUNT=$(echo "$RESULT" | jq "[${THREADS_JSON} | select(.body | startswith(\"SMOKE_OLD_\"))] | length")
NEW_COUNT=$(echo "$RESULT" | jq "[${THREADS_JSON} | select(.body == \"SMOKE_NEW_COMMENT\")] | length")

echo "Found $OLD_COUNT old comments, $NEW_COUNT new comments"

if [ "$OLD_COUNT" -lt 2 ]; then
  echo "FAIL: Expected at least 2 old comments, found $OLD_COUNT"
  echo "$RESULT" | jq '.data.repository.pullRequest.reviewThreads'
  exit 1
fi

if [ "$NEW_COUNT" -lt 1 ]; then
  echo "FAIL: Expected at least 1 new comment, found $NEW_COUNT"
  echo "$RESULT" | jq '.data.repository.pullRequest.reviewThreads'
  exit 1
fi

# Verify old comments are minimized
OLD_ALL_MINIMIZED=$(echo "$RESULT" | jq "
  [${THREADS_JSON}
   | select(.body | startswith(\"SMOKE_OLD_\"))
   | .isMinimized] | all")

# Verify new comment is NOT minimized
NEW_NOT_MINIMIZED=$(echo "$RESULT" | jq "
  [${THREADS_JSON}
   | select(.body == \"SMOKE_NEW_COMMENT\")
   | .isMinimized] | map(. == false) | all")

echo "Old comments all minimized: $OLD_ALL_MINIMIZED"
echo "New comment kept visible:   $NEW_NOT_MINIMIZED"

if [ "$OLD_ALL_MINIMIZED" != "true" ]; then
  echo "FAIL: Old review comments should be minimized"
  echo "$RESULT" | jq '.data.repository.pullRequest.reviewThreads'
  exit 1
fi

if [ "$NEW_NOT_MINIMIZED" != "true" ]; then
  echo "FAIL: New review comment should NOT be minimized"
  echo "$RESULT" | jq '.data.repository.pullRequest.reviewThreads'
  exit 1
fi
